#!/usr/bin/env python3
"""Train the char-Markov code-switch language detector bundled with euskaphone.

Four character-level Markov models (Basque, Spanish, French, English) are trained
on Wikipedia text and written gzip-compressed to
``euskaphone/data/langdetect/<lang>.json.gz`` — the artifacts
:mod:`euskaphone.langdetect` loads.
...
LANGS = ["eu", "es", "fr", "en"]
BOS, EOS = "\x02", "\x03"
OUT_DIR = Path(__file__).resolve().parent.parent / "euskaphone" / "data" / "langdetect"
