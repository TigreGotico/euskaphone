...
:mod:`euskaphone.langdetect` and the orthographic backstop in
:mod:`euskaphone.codeswitch` — and which one runs depends on whether the
optional ``markovonnx`` dependency is installed. A routing invariant has to hold
under both, so tests asserting one take the ``classifier`` fixture and run twice.
"""
import pytest
...
def classifier(request, monkeypatch):
    ...
    """
    import euskaphone.langdetect as langdetect
